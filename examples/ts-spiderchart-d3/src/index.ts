...
>(mod: Spotfire.Mod, rowLimit = 2000): (a: T) => T {
...
async function buildData(mod: Mod, dataView: DataView): Promise<Data> {
  ...
  const allYValues: Array<number> = allRows!.map(
    (row) => row.continuous<number>("Y").value() || 0
  );
  ...
  const colorLeaves = (await (await dataView.hierarchy(
    "Color"
  ))!.root())!.leaves();
  ...
  function createPointTooltip(point: Point) {
    ...
    if (yAxisMeta.parts.length > 1) {
      ...
      let colorLevelForColumnNames = colorAxisMeta.parts
        .map((p) => p.expression)
        .indexOf("[Axis.Default.Names]");
      let xLevelForColumnNames = xAxisMeta.parts
        .map((p) => p.expression)
        .indexOf("[Axis.Default.Names]");
    ...
    return [
      createAxisTooltip(xAxisMeta.parts, xValues, separator) ||
        xAxisMeta.parts[0].displayName +
          ": " +
          xHierarchyLeaves[point.xIndex].formattedPath(),
      ...
      colorAxisMeta.parts.length > 0
        ? createAxisTooltip(colorAxisMeta.parts, colorValues, separator) ||
          colorAxisMeta.parts[0].displayName +
            ": " +
            colorLeaves[point.index].formattedPath()
        : "",
    ].join(separator);
  }
  ...
  function createAxisTooltip(
    ...
    return axisParts.length == formattedValues.length
      ? formattedValues
          .map((v, i) => axisParts[i].displayName + ": " + v)
          .join(separator)
      : null;
  }
}
